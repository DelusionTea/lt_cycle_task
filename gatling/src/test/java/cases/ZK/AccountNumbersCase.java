...
public class AccountNumbersCase extends Methods {

    private static final String JSONS_PATH = "JSONs/ZK/";

    public static HttpRequestActionBuilder UC01_GET_v2_account_numbers =
            http("UC01_GET_/api/v2/account-numbers")
                    .get("/api/v2/account-numbers")
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC02_POST_v2_account_numbers =
            http("UC02_POST_/api/v2/account-numbers")
                    .post("/api/v2/account-numbers")
                    .body(ElFileBody(JSONS_PATH + "AccountNumbers/ComplianceAccountNumberApiModel_body.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC03_PATCH_v2_account_numbers_batch_identical =
            http("UC03_PATCH_/api/v2/account-numbers/batch-identical")
                    .patch("/api/v2/account-numbers/batch-identical")
                    .body(ElFileBody(JSONS_PATH + "AccountNumbers/ComplianceAccountNumberApiModel_body.json"))
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));
