...
public class FinOperationsAttributesCase extends Methods {

    private static final String JSONS_PATH = "JSONs/ZK/";

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC01_GET_v2_fin_operations_attributes =
            http("UC01_GET_/api/v2/fin-operations-attributes")
                    .get("/api/v2/fin-operations-attributes")
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC02_POST_v2_fin_operations_attributes =
            http("UC02_POST_/api/v2/fin-operations-attributes")
                    .post("/api/v2/fin-operations-attributes")
                    .body(ElFileBody(JSONS_PATH + "FinOperationsAttributes/ComplianceFinOperationAttributeApiModel_body.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC03_PATCH_v2_fin_operations_attributes_batch_identical =
            http("UC03_PATCH_/api/v2/fin-operations-attributes/batch-identical")
                    .patch("/api/v2/fin-operations-attributes/batch-identical")
                    .body(ElFileBody(JSONS_PATH + "FinOperationsAttributes/ComplianceFinOperationAttributeApiModel_body.json"))
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));
