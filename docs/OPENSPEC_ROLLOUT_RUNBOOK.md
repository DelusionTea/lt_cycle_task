...
`OpenSpec artifact -> openspec_preflight.py -> spec_to_props/spec_to_profile -> Jenkins/Gatling`.
...
1. Run preflight (`openspec_preflight.py`): convert + validate + dry profile generation.
2. Generate `profile.properties`.
...
python3 SDD/ltAuto/openspec_preflight.py \
  --spec tmp/openspec-smoke/spec.yaml \
  --expected_service ZK \
  --use_openspec \
  --openspec SDD/specs/ZK/ComplianceRequests/spec.yaml \
  --target_percent 100 \
  --dry_profile_props \
  --dry_profile_yaml
