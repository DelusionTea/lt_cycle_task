...
- **`count` — запросов/час на 100% профиля** (`target_percent` масштабирует интенсивность прогона).
...
| Источник | Когда |
|----------|-------|
| Бизнес-цель / старый профиль | перенос существующего значения (в req/h) |
| `sim_to_profile.py` | есть веса + RPS/throttle — **предпочтительно** |
| `ok_requests` из log_labels | ориентир с прогона: `count ≈ ok × 3600 / T_сек × (100 / target_percent)` |
