...
```
count_i = round(RPS_total × вес_i / Σвесов × длительность_сек)
```
...
```
- [ ] 1. Найти RPS_total и длительность (throttle: reachRps / holdFor)
- [ ] 2. Запустить sim_to_profile.py
- [ ] 3. Проверить: сумма весов, все запросы на месте, суммарный count/сек ≈ RPS
...
**3. Верификация чисел.** Сумма `count_i / duration` должна давать ≈ `RPS_total`;
...
**5. Проверка совместимости** — профиль должен читаться генератором properties:
```bash
python3 ltAuto/profile_to_props.py --profile profiles/profile.generated.yaml --target_percent 100 --output /tmp/check.properties
```
Ошибок быть не должно; веса в `/tmp/check.properties` должны совпасть по долям с
исходными весами сценария (замкнутая проверка «веса → count → веса»).
