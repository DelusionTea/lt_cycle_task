...
- `meta.service` должен совпадать с ключом АС в `profiles/grafana.yaml` и параметром
  `APPLICATION` в Jenkins. Для каждой АС в `profiles/grafana.yaml` должны быть
  указаны дашборды, по которым снимаются скриншоты.
