...
public class ConstructorPfParametersCase extends Methods {

    private static final String JSONS_PATH = "JSONs/ZK/";

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC01_GET_v1_constructor_pf_parameters =
            http("UC01_GET_/api/v1/constructor-pf-parameters")
                    .get("/api/v1/constructor-pf-parameters")
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC02_POST_v1_constructor_pf_parameters =
            http("UC02_POST_/api/v1/constructor-pf-parameters")
                    .post("/api/v1/constructor-pf-parameters")
                    .body(ElFileBody(JSONS_PATH + "ConstructorPfParameters/ConstructorPfParameterApiModel_body.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC03_PATCH_v1_constructor_pf_parameters_batch_identical =
            http("UC03_PATCH_/api/v1/constructor-pf-parameters/batch-identical")
                    .patch("/api/v1/constructor-pf-parameters/batch-identical")
                    .body(ElFileBody(JSONS_PATH + "ConstructorPfParameters/ConstructorPfParameterApiModel_body.json"))
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));
