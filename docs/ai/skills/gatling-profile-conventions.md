...
## Реальная раскладка репозитория

Скрипты и профили — в `gatling/gatlingScripts/`; запускай из этого каталога:
`ltAuto/*.py`, `profiles/*.yaml`, Case-классы `src/test/java/cases/<домен>/`.
