...
public class DigitalOfficesCase extends Methods {

    private static final String JSONS_PATH = "JSONs/ZK/";

    public static HttpRequestActionBuilder UC01_GET_v2_digital_offices =
            http("UC01_GET_/api/v2/digital-offices")
                    .get("/api/v2/digital-offices")
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC02_POST_v2_digital_offices =
            http("UC02_POST_/api/v2/digital-offices")
                    .post("/api/v2/digital-offices")
                    .body(ElFileBody(JSONS_PATH + "DigitalOffices/DigitalOfficeApiModel_body.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC03_PATCH_v2_digital_offices_batch_identical =
            http("UC03_PATCH_/api/v2/digital-offices/batch-identical")
                    .patch("/api/v2/digital-offices/batch-identical")
                    .body(ElFileBody(JSONS_PATH + "DigitalOffices/DigitalOfficeApiModel_body.json"))
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));
