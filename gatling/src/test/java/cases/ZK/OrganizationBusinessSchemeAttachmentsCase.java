...
public class OrganizationBusinessSchemeAttachmentsCase extends Methods {

    private static final String JSONS_PATH = "JSONs/ZK/";

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC01_GET_v1_organization_business_scheme_attachments =
            http("UC01_GET_/api/v1/organization-business-scheme-attachments")
                    .get("/api/v1/organization-business-scheme-attachments")
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC02_POST_v1_organization_business_scheme_attachments =
            http("UC02_POST_/api/v1/organization-business-scheme-attachments")
                    .post("/api/v1/organization-business-scheme-attachments")
                    .body(ElFileBody(JSONS_PATH + "OrganizationBusinessSchemeAttachments/OrganizationBusinessSchemeAttachmentApiModel_body.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));

    // Низконагружен не попал в профиль
    public static HttpRequestActionBuilder UC03_PATCH_v1_organization_business_scheme_attachments_batch_identical =
            http("UC03_PATCH_/api/v1/organization-business-scheme-attachments/batch-identical")
                    .patch("/api/v1/organization-business-scheme-attachments/batch-identical")
                    .body(ElFileBody(JSONS_PATH + "OrganizationBusinessSchemeAttachments/OrganizationBusinessSchemeAttachmentApiModel_body.json"))
                    .queryParam("filter", Methods.queryFromFile(JSONS_PATH + "shared/PagingRequestExt_filter.json"))
                    .headers(Headers.getCommonHeaders())
                    .check(status().is(200));
